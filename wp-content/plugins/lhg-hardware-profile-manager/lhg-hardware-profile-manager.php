...
Version: 0.2
Plugin URI: https://github.com/cptpike/linuxhardwareguide/
Description: "LHG - Hardware Profile Manager" is an adapted version of "Subscribe to Comments Reloaded" which allows to create hardware profiles for users
Author: cptpike
...
function subscribe_reloaded_show(){
        global $txt_subscr_answer;
        global $txt_subscr_answer_logged_in;
	...
	load_plugin_textdomain('subscribe-reloaded', WP_PLUGIN_DIR .'/lhg-hardware-profile-manager/langs', '/lhg-hardware-profile-manager/langs');
	...
	if ($show_subscription_box){
		...
		$checkbox_html_wrap = html_entity_decode(stripslashes(get_option('subscribe_reloaded_checkbox_html', '')), ENT_COMPAT, 'UTF-8');
			...
			$checkbox_field_active = "<input$checkbox_inline_style type='checkbox' name='subscribe-reloaded' id='subscribe-reloaded' value='yes' checked='checked' />";
		}
		else{
		...
		if (empty($checkbox_html_wrap)){
			$html_to_show = "$checkbox_field <label for='subscribe-reloaded'>$checkbox_label</label>" . $html_to_show;
                	...
                	//user not logged in: unchecked + short text
        	        //user logged in: checked by default + long text

			if (  is_user_logged_in() ) $checkbox_html_wrap = str_replace('[checkbox_field]', $checkbox_field_active, $checkbox_html_wrap);
			if ( !is_user_logged_in() ) $checkbox_html_wrap = str_replace('[checkbox_field]', $checkbox_field, $checkbox_html_wrap);
			if (  is_user_logged_in() ) $html_to_show = str_replace('[checkbox_label]', "$txt_subscr_answer_logged_in", $checkbox_html_wrap) . $html_to_show;
			if ( !is_user_logged_in() ) $html_to_show = str_replace('[checkbox_label]', "$txt_subscr_answer", $checkbox_html_wrap) . $html_to_show;
			//$html_to_show = str_replace('[checkbox_label]', "$txt_answer", $checkbox_html_wrap) . $html_to_show;
			$html_to_show = str_replace('<p>', "", $html_to_show);
			$html_to_show = str_replace('<p >', "", $html_to_show);
			$html_to_show = "<div class='subscribe-reloaded'>".'<span style="white-space: nowrap">'.
                        str_replace('</p>', "", $html_to_show)
                        ."</span></div><br clear='left' />";
...
class wp_subscribe_reloaded{
	...
	private function _activate(){
		...
		add_option('subscribe_reloaded_checkbox_html', "<p><label for='subscribe-reloaded' class='subscribe-reloaded'>[checkbox_field] [checkbox_label]</label></p>", '', 'no');
		...
		if (!empty($_POST['subscribe-reloaded']) && !empty($info->comment_author_email)){
			...
			if (!$this->is_user_subscribed($info->comment_post_ID, $info->comment_author_email)){
				...
				$approved_subscriptions = $this->get_subscriptions(array('status', 'email'), array('equals', 'equals'), array('Y', $info->comment_author_email));
                                ...
                                // no double check-in for logged in users
	...
	public function add_config_menu( $_s ) {
		...
		if (current_user_can('manage_options')){
			add_options_page('Subscribe to Comments', 'Subscribe to Comments', 'manage_options', WP_PLUGIN_DIR.'/lhg-hardware-profile-manager/options/index.php');
		}
		return $_s;
	}
	// end add_config_menu
	...
	public function add_options_stylesheet(){
		...
		$stylesheet_url = (is_ssl()?str_replace('http://', 'https://', WP_PLUGIN_URL):WP_PLUGIN_URL).'/lhg-hardware-profile-manager/style.css';
	...
	public function add_post_comments_stylesheet(){
		// It looks like WP_PLUGIN_URL doesn't honor the HTTPS setting in wp-config.php
		$stylesheet_url = (is_ssl()?str_replace('http://', 'https://', WP_PLUGIN_URL):WP_PLUGIN_URL).'/lhg-hardware-profile-manager/post-and-comments.css';
		wp_register_style('subscribe-to-comments', $stylesheet_url);
		wp_enqueue_style('subscribe-to-comments');
	}
	// end add_stylesheet
	...
	public function add_column_header($_columns){
		...
		$image_url = (is_ssl()?str_replace('http://', 'https://', WP_PLUGIN_URL):WP_PLUGIN_URL).'/lhg-hardware-profile-manager/images';
	...
	public function contextual_help($contextual_help, $screen_id, $screen){
		if ($screen_id == 'lhg-hardware-profile-manager/options/index'){
			load_plugin_textdomain('subscribe-reloaded', WP_PLUGIN_DIR .'/lhg-hardware-profile-manager/langs', '/lhg-hardware-profile-manager/langs');
			$contextual_help = __('Need help on how to use Subscribe to Comments Reloaded? Visit the official','subscribe-reloaded').' <a href="http://wordpress.org/tags/lhg-hardware-profile-manager?forum_id=10" target="_blank">'.__('support forum','subscribe-reloaded').'</a>. ';
