...
class WPSmart
{
	...
	function wps_check_mobile()
	{
		global $wps_user_agents;
                ...
                global $domain_name_is_mobile;
                global $was_redirected; //check if we are initiated by redirected page

                $lhg_is_mobile_mode = 0;
                $lhg_mobile_size = "small";



                //check for domain name
                $domain_name = $_SERVER['HTTP_HOST'];
                $domain_name_is_mobile = 0;
                $cookie_domain = str_replace("www","",$domain_name);
                $cookie_domain = str_replace("m.",".",$cookie_domain);

                #$log2 = "CookieDom: $cookie_domain \n";
                #$log2 .= "Dom: $domain_name \n";
                #$log2 .= "Was redirected: $was_redirected  \n";

		#$log = fopen("/tmp/user_agents.log", 'a') or die("can't open file");
		#fwrite($log,$server_user_agent."\n");
		#fwrite($log,$log2."\n");
                #fclose($log);


                if ( substr($domain_name, 0, 2) == "m." ) $domain_name_is_mobile = 1;

                #echo "-------------------> DOM: $domain_name - $domain_name_is_mobile";

                /*
		if( ! is_admin()  and ($was_redirected != 1) ) {

	               # echo "<br><br><br>1. -----------------> Cookie NOMV: ".$_COOKIE['Now_On_Mobile_Version']."<br>";
        	       # echo "-----------------> Cookie wp: ".$_COOKIE['wpsmart_view_full_site']."<br>";
                       # echo "-----------------> Domain name: $domain_name ($domain_name_is_mobile)<br>";


                        //echo "TEST: AA<br>";
                        //echo "COOKIE: ".$_COOKIE['wpsmart_view_full_site'];
			...
			if( wps_is_in_preview_mode() ) {
				...
				$this->wps_show_mobile = true;
                                $lhg_is_mobile_mode = 1;
			}

                        //first visit? check if redirect is necessary

			if( isset($_COOKIE['Now_On_Mobile_Version']) or isset( $_COOKIE['wpsmart_view_full_site'] ) ) {
                                ...
                                if ($domain_name_is_mobile == 1) {
                                        #echo "Opt 1";
                                	$lhg_is_mobile_mode = 1;
                                        $this->wps_show_mobile = true;

	                                setcookie("Now_On_Mobile_Version",1,time()+3600*2, "/", $cookie_domain);
				}
                                ...
                                if ($domain_name_is_mobile != 1) {
                                	...
                                	$lhg_is_mobile_mode = 0;
                                        ...
                                        setcookie("Now_On_Mobile_Version",0,time()+3600*2, "/", $cookie_domain);
                                        wps_redirect_to_www();
                          ...
                          */
                                // was never here
                                // we redirect the visitor

                                #echo "================> CHECK".wps_check_for_mobile_device();

                  // simplified version - show mobile version on m.linux-hardware-guide.com/de.
                  // show full verison always else - no client recognition, no cookie tricks
                  // Instead, responsive design should be implemented for www.
                  if ($domain_name_is_mobile == 1) {

                                        // Visitor came straight to the mobile version
                                        // follow his wish
                                	$this->wps_show_mobile = true;
	                                $lhg_is_mobile_mode = 1;
                  ...
                  /*
                                	...
                                	if (wps_check_for_mobile_device() == 0) {
						$this->wps_show_mobile = false;
        		                        $lhg_is_mobile_mode = 0;
					}else{
                                		$this->wps_show_mobile = true;
	                                	$lhg_is_mobile_mode = 1;
	                                        #echo "---------------------->redirect!<br>";
        	                                lhg_redirect_to_mobile();
                	                        setcookie("Now_On_Mobile_Version",1,time()+3600*2);
                        	        }
  ...
  /*
                        if( ($domain_name_is_mobile == 0) && $_COOKIE['wpsmart_view_full_site'] == 1 ) {
                                // echo "CC";
                                // on www and wants full view
				$this->wps_show_mobile = false;
                                $lhg_is_mobile_mode = 0;

			} elseif( ($domain_name_is_mobile == 0) && $_COOKIE['Now_On_Mobile_Version'] == 1 ) {
                                echo "----------------------> 01<br>";
                                // was on mobile version, must have switched to www version
				$this->wps_show_mobile = false;
                                $lhg_is_mobile_mode = 0;

			} elseif( ($domain_name_is_mobile == 0) && (!isset($_COOKIE['wpsmart_view_full_site'] ) ) ) {
                                #echo "BB";
                                // on www but first visit

			} elseif( ($domain_name_is_mobile == 0) && ( $_COOKIE['wpsmart_view_full_site'] == 0 ) )  {
                                //echo "AA";
                                // on www but wanted mobile
                                $this->wps_show_mobile = true;
                                $lhg_is_mobile_mode = 1;
                                wps_redirect_to_mobile();

			//} elseif( ( isset( $_COOKIE['wpsmart_view_full_site'] ) && $_COOKIE['wpsmart_view_full_site'] == 1 ) && $domain_name_is_mobile == 0 ) {
			//	$this->wps_show_mobile = false;
			//	add_action( 'wp_footer', array( &$this, 'wps_view_mobile_site' ) );

			}

                echo "<br><br><br>2. -----------------> Cookie NOMV: ".$_COOKIE['Now_On_Mobile_Version']."<br>";
                echo "-----------------> Cookie wp: ".$_COOKIE['wpsmart_view_full_site']."<br>";
                echo "-----------------> Domain name: $domain_name_is_mobile<br>";
		}
	    */
	}
...
function wps_check_for_mobile_device ( )
	{

        global $lhg_is_mobile_mode;
        global $domain_name_is_mobile;
        global $wp_round_mobile;

        $wp_round_mobile = $wp_round_mobile+ 1;
#	#if ($domain_name_is_mobile == "") return 0;
#        $log2 = "---------------------> TESTing client";
#
#        $log2 .= "
#0. -----------------> Cookie NOMV: ".isset($_COOKIE['Now_On_Mobile_Version'])."
#   -----------------> Cookie wp: ".$_COOKIE['wpsmart_view_full_site']."
#   -----------------> Domain name:".$domain_name_is_mobile."
#   -----------------> LHG Mode:".$lhg_is_mobile_mode."
#   -----------------> WP Round:".$wp_round_mobile."
#   ";


        $server_user_agent = strtolower($_SERVER['HTTP_USER_AGENT']);

        #echo "---------------------->UA: $server_user_agent<br>";
	global $wps_user_agents;

	#print_r($wps_user_agents);
        #echo "<br>";

	foreach( $wps_user_agents as $user_agent ) {
#        	$log2 .= "--------------------------->SEARCH: $user_agent
#                ";
		if( preg_match( "/$user_agent/", $server_user_agent) ) {

#                        $log2 .="--------------------------->FOUND: $user_agent
#                        ";
			...
			#$this->wps_show_mobile = true;
                	...
                	//define banner size
			if( preg_match( "/ipad/", $server_user_agent) ) {
                		$lhg_mobile_size = "large";
			}


			if( preg_match( "/htc legend/", $server_user_agent) ) {
                	 	$lhg_mobile_size = "small";
			}
...
function wps_redirect_to_mobile( )
{

        //prevent double redirect
        $domain_name = $_SERVER['HTTP_HOST'];
	if ( substr($domain_name, 0, 2) == "m." ) return;


	//$actual_link_full = "http://".str_replace("m.","",$_SERVER[HTTP_HOST]).$_SERVER[REQUEST_URI];
	$actual_link_mobile = "http://m.".str_replace("www.","",$_SERVER[HTTP_HOST]).$_SERVER[REQUEST_URI];


	//if ($lang != "de")

        //set marker that redirection was carried out (no second check necessary!)
        global $was_redirected;
        $was_redirected = 1;

        header("Location: ".$actual_link_mobile);
        //if ($lang == "de") header("Location: http://m.linux-hardware-guide.de/");
        exit;
}

function lhg_redirect_to_mobile( )
{
        //prevent double redirect
        $domain_name = $_SERVER['HTTP_HOST'];
	if ( substr($domain_name, 0, 2) == "m." ) return;
...
function wps_redirect_to_www( )
{
        //prevent double redirect
        $domain_name = $_SERVER['HTTP_HOST'];
	if ( substr($domain_name, 0, 4) == "www." ) return;


	//$actual_link_full = "http://".str_replace("m.","",$_SERVER[HTTP_HOST]).$_SERVER[REQUEST_URI];
	$actual_link_mobile = "http://www.".str_replace("m.","",$_SERVER[HTTP_HOST]).$_SERVER[REQUEST_URI];
