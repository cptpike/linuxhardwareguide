...
global $txt_at;//                 = "um";
global $txt_product_rating;//     = "Produkt-Bewertung";
...
function get_comment_reply_link($args = array(), $comment = null, $post = null) {
	...
	$defaults = array('add_below' => 'comment', 'respond_id' => 'respond', 'reply_text' => __($txt_reply),
	...
	if ( get_option('comment_registration') && !$user_ID )
		$link = '<a rel="nofollow" class="comment-reply-login" href="' . esc_url( wp_login_url( get_permalink() ) ) . '">' . $login_text . '</a>';
                ...
                //<button type="submit" value="Hinzuf&uuml;gen" />Hinzuf&uuml;gen&nbsp;<i class="icon-arrow-right icon-button"></i></button>
		$link = "<a class='comment-reply-link' href='" . esc_url( add_query_arg( 'replytocom', $comment->comment_ID ) ) . "#" . $respond_id . "' onclick='return addComment.moveForm(\"$add_below-$comment->comment_ID\", \"$comment->comment_ID\", \"$respond_id\", \"$post->ID\")'>$reply_text <i class='icon-arrow-right icon-button'></i></a>";
...
class Walker_Comment extends Walker {
	...
	function start_el(&$output, $comment, $depth, $args) {
		...
		$lang_url = lhg_get_lang_url_from_region ( $region );
                ...
                if ($lang_url != "") $lang_url = "/".$lang_url;
            	if ($uid != 0) $tooltip = lhg_user_tooltip( $uid );
                $user_link     = '<a href="'.$lang_url.'/hardware-profile/user'.$uid.'" class="userlink" title="'.$tooltip.'"></a>';
                $user_link_end = '';//</a>';
                if ($uid != 0) $output_tmp_1 = $user_link;
            	if ($uid != 0) $output_tmp_2 = $user_link_end;

                $percent_bar="";
                if ($uid != 0) $percent_bar = lhg_comment_percent_bar($uid);
...
<?php endif; ?>
		...
		<div class="comment-meta commentmetadata"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>">
			...
			<?php
				...
				printf( __('%1$s '.$txt_at.' %2$s'), get_comment_date(),  get_comment_time()) ?></a><?php
...
<?php
	...
	function end_el(&$output, $comment, $depth, $args) {
        ...
        global $last_reply_field;
        //echo "Test-End";
        //var_dump($last_reply_field);
        //echo "DP: $depth";
        if ($depth == 0)
        echo "</div>".$last_reply_field;
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$required_text = sprintf( ' ' . __('Required fields are marked %s'), '<span class="required">*</span>' );
        ...
        if ($out1 == "") $out1 = "Error: No rating found";

        $lhg_rating_line="";
        if ( is_single() ) $lhg_rating_line='
		...
		'comment_field'        => sprintf( $tmp1 ),
		'must_log_in'          => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( $txt_logged_in_as.' <a href="%1$s">%2$s</a>. <a href="%3$s" title="Abmelden">'.$txt_logout.'?</a>' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'comment_notes_before' => '<p class="comment-notes">' . __( $txt_privacy_stat ) . ( $req ? $required_text : '' ) . '</p>',
		'comment_notes_after'  => '<p class="form-allowed-tags">' . sprintf( __( $txt_use_tags.': %s' ), ' <code>' . allowed_tags() . '</code>' ) . '</p>',
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
		'title_reply'          => __( "$txt_comment " ),
		'title_reply_to'       => __( $txt_reply_to.' %s' ),
		'cancel_reply_link'    => __( $txt_cancel_reply ),
		'label_submit'         => __( $txt_send_comment ),
	);
	...
	?>
		<?php if ( comments_open() ) : ?>
			...
			<div id="respond">
				...
				<?php else : ?>
					<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" enctype="multipart/form-data" id="<?php echo esc_attr( $args['id_form'] ); ?>">
						...
						<p class="form-submit">
							<button name="submit_comments" type="submit" value="<?php echo esc_attr( $args['label_submit'] ); ?>" ><?php echo esc_attr( $args['label_submit'] ).'&nbsp;<i class="icon-arrow-right icon-button"></i>'; ?></button>
